...
#Python script to run MPRA Pipeline Analysis on Ruddle
...
def get_args():
	...
	parser.add_argument(
		'-s', '--sample', type=str, help="Sample Type. Must specify if in EXP mode ex. Rep2_1_pDNA", required=False)
	...
	parser.add_argument(
		'-I', '--INERTtag', type=str, help="Inert tag counts, must specify when in annotate mode", required=False)
	parser.add_argument(
		'-C', '--COMPtag', type=str, help="Competent Library Seq Tag Counts Optional to specify in annotate mode", required=False)
	parser.add_argument(
	...
	return Mode, ROne, RTwo, Samp, ID, out, inertTAG, compTAG, plasTAG, tranTAG, resource

#Match return values from get_arguments() and assign to their respective variables
Mode, ROne, RTwo, Samp, ID, out, inertTAG, compTAG, plasTAG, tranTAG, resource = get_args()
...
##Og Script call: python ~/Scripts/Strander.py HC_RQ5138_R1.fastq HC_RQ5138_R2.fastq
...
#Define the Function that matches Cres to Tags
##OG Matcher Call: python ~/Scripts/Matcher.py Cres.fastq Tags.fastq
...
####Changed: ~/Scripts/LibTranslation.txt to /home/jkg36/Genomes/MPRA/Lib_Align_Files/Condensed_HC_Lib/Masked_LibTranslation.txt
####Changed: print >>Parsed, name,"\t",UniqID -> print >>Parsed, name,"\t",KEY[UniqID]
#### Skips importing Genomic Align MPRA Translator Script
####Changed: Cigar_Tag_Anote.py rolled into this def too_ will print out tags in final file
####Compiled_Cre_tags = Inert_Compiled_Cre_tags.txt
def GenomicParse(bamFile, Translator, Compiled_Cre_tags):
...
def MultipleTagged( File ):			
	##Original Script Call: Orginal Script Call python ~/Scripts/MultipleTagged.py Translated_Parsed_Cigar_withTAG_TrimedPI_gt88_temp.txt
	F1 = open(File)
...
##Og Usage: python ~/Scripts/TAG_Strander_V2016_10_31.py R1_Trimmed.fastq R2_Trimmed.fastq
...
#Tag Tabulator script: used for both Competent Library and Experiment Seq call on "Tags.fastq"
##Original Usage time python Tag_Tabulator.py Tags.fastq 
def TagTabulator( File, Sample ):
	F1= open(File)
...
####Original use: python ~/Scripts/Common_Tags.py Competant_Counts.txt Inert_Counts.txt
...
##Original Usage time python ~/Scripts/Tag_Strander_2016_11_21.py R1_pDNA_Trimmed.fastq R2_pDNA_Trimmed.fastq
...
if (Mode == "INERT"):
	##Example Call:
	#python Pipeline.py -r INERT -PI .88 -R1 Inert_R1.fastq.gz -R2 Inert_R2.fastq.gz -o Inert_Pipeline_OUT.txt
	...
	if ID is None:
		...
		if (ID <= 0 or ID > 1):
			...
			IndexName = ''.join([resource,"HC_GenomicBased_Library_Bwtie2Index"])
			print IndexName
			...
			GenomicParse( "Inert.bam", TransName ,"Inert_Compiled_Cre_tags.txt" ) 
...
####Competent Library Processing pipeline
	...
	##Example Call:
	#python Pipeline.py -r COMP -R1 Comp_R1.fastq.gz -R2 Comp_R2.fastq.gz -o Comp_Pipeline_OUT.txt
	...
	if ROne is None:
		...
		TagTabulator( "Comp_Tags.fastq", "CompLib" )
	
####Experimental Read Processing Pipeline
elif (Mode == "EXP"):
	##Example Call
	#python Pipeline.py -r EXP -s pDNA_Test -R1 pDNA_R1.fastq.gz -R2 pDNA_R2.fastq.gz -o pNDA_EXP_Pipeline_OUT.txt
	#python Pipeline.py -r EXP -s cDNA_Test -R1 cDNA_R1.fastq.gz -R2 cDNA_R2.fastq.gz -o cNDA_EXP_Pipeline_OUT.txt
	...
	if Samp is None:
		...
		print >>PipeOUT, cmd
		subprocess.Popen(cmd, shell=True).wait()
		cmd = ''.join(["cutadapt -e 0.16 -a AGATCGGAAGAGCGTC -o ",Samp,"_Tags.fastq ",Samp,"_5P_Cut_Seqs.fastq > ",Samp,"3PrimeTrim_Log.txt"])
		print >>PipeOUT, cmd
		subprocess.Popen(cmd, shell=True).wait()
		
		#Tabulate Tag Counts
		TagTabulator( ''.join([Samp,"_Tags.fastq"]), Samp )
...
####Create a Master Annotation file from all Tag calls across replicates and competent library sequenceing	
elif (Mode == "ANOTE"):
	##Example Call:
